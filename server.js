...
app.post('/api/citizens/:id/wanted', async (req, res) => {
    ...
    const result = await pool.query(
        `UPDATE citizens SET is_wanted = TRUE, wanted_reason = $1, wanted_category = $2, wanted_priority = $3, wanted_since = NOW() WHERE id = $4 RETURNING *`,
...
app.post('/api/citizens/:id/unwanted', async (req, res) => {
    const { id } = req.params;
    const result = await pool.query(
        `UPDATE citizens SET is_wanted = FALSE, wanted_reason = NULL, wanted_category = NULL, wanted_priority = NULL, wanted_since = NULL WHERE id = $1 RETURNING *`,
        [id]
    );
    res.json(result.rows[0]);
});
...
app.get('/api/weapons', async (req, res) => {
    const result = await pool.query(`SELECT w.*, c.nickname as owner_nickname FROM weapons w LEFT JOIN citizens c ON w.owner_id = c.id ORDER BY w.id DESC`);
    res.json(result.rows);
});
...
app.get('/api/vehicles', async (req, res) => {
    ...
    const result = await pool.query(`SELECT v.*, c.nickname as owner_nickname FROM vehicles v LEFT JOIN citizens c ON v.owner_id = c.id ORDER BY v.id DESC`);
    res.json(result.rows);
});
...
app.get('/api/violations', async (req, res) => {
    const result = await pool.query(`SELECT v.*, c.nickname as citizen_nickname FROM traffic_violations v LEFT JOIN citizens c ON v.citizen_id = c.id ORDER BY v.created_at DESC`);
    res.json(result.rows);
});
...
app.get('/api/criminal', async (req, res) => {
    const result = await pool.query(`SELECT cr.*, c.nickname, c.full_name FROM criminal_records cr JOIN citizens c ON cr.citizen_id = c.id WHERE cr.is_active = true ORDER BY cr.created_at DESC`);
    res.json(result.rows);
});
...
app.post('/api/duty/start', async (req, res) => {
    ...
    const result = await pool.query(`INSERT INTO duty_shifts (server_id) VALUES ($1) RETURNING *`, [server_id]);
    res.json(result.rows[0]);
});
